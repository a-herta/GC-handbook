...
import { copyToClipboard, entityIcon, notNaN } from '@/utils'
...
class EntityCard extends React.Component<IProps, IState> {
  ...
  private updateCount(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const updated = this.state.showingCount ? { count: numeric } : { level: numeric }
  ...
  private addCount(positive: boolean, multiple: boolean) {
    ...
    const updated = this.state.showingCount ? { count: value } : { level: value }
    this.setState(updated)
  }
  ...
  private async summonAtPlayer(): Promise<void> {
    ...
    const level =
      typeof this.state.level == 'string' ? parseInt(this.state.level) : this.state.level
        ...
        <div className={'flex flex-col gap-2.5'}>
          <div className={'flex flex-row justify-between'}>
            <div className={'flex flex-col gap-2.5'}>
              ...
              <p className={'text-[16px]'}>{data?.type ?? ''}</p>
            </div>
